...
def generateDescription(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "generateAds/advertisement.html",
                {
                    "form": form,
                    "adsDf": df.to_html(
...
def generateAds(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "generateAds/advertisement.html",
            {"descriptive_form": descriptive_form, "large_form": large_form},
        )
...
def generate(request, products=["jack"], max_length=100, fallback="Great Cities"):
    if request.is_ajax() and request.method == "POST":
        ...
        return JsonResponse({"success": True, "result": ads_gen})
    else:
        return JsonResponse({"sucess": False, "result": "Invalid request"})
...
def generateKeywords(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "generateAds/keywords.html",
                {
                    "form": form,
                    "keywordDf": keywordDf.to_html(
                        classes="table table-striped text-center", justify="center"
                    ),
                },
            )
